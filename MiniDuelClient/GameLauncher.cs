...
namespace MiniDuelClient
{
    enum GameLauncherMode
    {
        ...
        Inject,
        /// <summary>
        /// Launch the process normally but provide args to be picked up by the MiniDuel BepInEx plugin
        /// </summary>
        BepInEx
    }
    ...
    class GameLauncher
    {
        ...
        public const string LoaderDll = "MiniDuelLoader.dll";

        public static string GetLoaderDllPath()
        {
            return Path.GetFullPath(Path.Combine(Program.CurrentDir, LoaderDll));
        }

        public static string GetGameDirectory()
        {
            string configuredGameDirectory = ClientSettings.GameDirectory;
            if (!string.IsNullOrEmpty(configuredGameDirectory) && Directory.Exists(configuredGameDirectory))
            {
                return configuredGameDirectory;
            }

            string legacyGameDirectory = Path.GetFullPath(Path.Combine(Program.CurrentDir, ".."));
            return Directory.Exists(legacyGameDirectory) ? legacyGameDirectory : configuredGameDirectory;
        }

        public static string GetGameExePath()
        {
            return Utils.GetGameExecutablePath(GetGameDirectory());
        }

        public static string GetGameDuelDllPath()
        {
            return Utils.GetGameDuelDllPath(GetGameDirectory());
        }

        public static unsafe bool Launch(GameLauncherMode mode, params string[] args)
        {
            string exeDir = null;
            string dllPath = GetLoaderDllPath();
            ...
            {
                ProcessStartInfo startInfo = new ProcessStartInfo(exePath, "--MiniDuelDir=\"" + Program.CurrentDir + "\"");
