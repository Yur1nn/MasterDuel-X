...
HRESULT LoadDotNetImpl()
{
    ...
    if (MonoExists(monoDllPath) && LoadMono(monoDllPath, "MiniDuelClient.exe", runningLive ? "live" : ""))
    {
        return 0;
    }
#endif
    ...
    GetRelativeFilePath(binaryPath, L"\\MiniDuelClient.exe");
    if (!FileExistsW(binaryPath))
    {
        return 0x1333337;
    }
    result = runtimeHost->ExecuteInDefaultAppDomain(binaryPath, L"MiniDuelClient.Program", L"DllMain", runningLive ? L"live" : NULL, NULL);
    return result;
}
...
int CreateHooks()
{
    ...
    BOOL isBepInEx = commandLine && wcsstr(commandLine, L"--MiniDuelDir=\"") != NULL;
