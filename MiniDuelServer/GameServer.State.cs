...
namespace MiniDuel
{
    partial class GameServer
    {
        ...
        void LoadSettings()
        {
            ...
            RuntimePaths.ServerExecutablePath = ygoMasterExePath;
            dataRootDirectory = Utils.GetDataDirectory(true, Path.GetDirectoryName(ygoMasterExePath));
            if (!Directory.Exists(dataRootDirectory))
            {
                Utils.LogWarning("Failed to find data directory '" + dataRootDirectory + "'");
                return;
            }
            dataDirectory = Utils.GetSharedDataDirectory(dataRootDirectory);
            clientDataDirectory = Utils.GetClientDataDirectory(dataRootDirectory);
            serverDataDirectory = Utils.GetServerDataDirectory(dataRootDirectory);
            settingsFile = Path.Combine(serverDataDirectory, "Settings.json");
            playersDirectory = Utils.GetPlayersDirectory(dataRootDirectory);
            ...
            string clientSettingsFile = Path.Combine(clientDataDirectory, "ClientSettings.json");
            if (File.Exists(clientSettingsFile))
            {
                ...
                if (clientValues != null)
                {
                    ...
                    {
                        clientSettingsTextFile = Path.Combine(clientDataDirectory, clientSettingsTextFile);
                    ...
                    string unityPlayerFile = Utils.GetUnityPlayerPath(RuntimePaths.GameDirectory);
            ...
            gameDirectory = Utils.ResolvePath(Path.GetDirectoryName(ygoMasterExePath), Utils.GetValue<string>(values, "GameDirectory", ".."));
            RuntimePaths.GameDirectory = gameDirectory;
            Utils.EnsureGameDllDirectory(gameDirectory);
            ...
            {
                FileInfo duelDllFile = new FileInfo(Utils.GetGameDuelDllPath(gameDirectory));
        ...
        void ConvertLeDataToSolo()
        {
            ...
            string dfymooText = "";
            string dfymooFile = Path.Combine(clientDataDirectory, "LinkEvolution", "chars.dfymoo");
