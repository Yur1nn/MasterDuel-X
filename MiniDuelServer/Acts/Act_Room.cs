...
namespace MiniDuel
{
    partial class GameServer
    {
        ...
        void Act_RoomBattleReady(GameServerWebRequest request)
        {
            ...
            string duelDllPath = Pvp.GetDllPath();
            if (!File.Exists(duelDllPath))
            {
                Utils.LogWarning("[Act_RoomBattleReady] Failed to find '" + duelDllPath + "'");
            ...
            if (request.Player == p1)
            {
                using (Process process = new Process())
                {
                    ...
                    if (Program.IsMonoRun)
                    {
                        ...
                        process.StartInfo.FileName = Path.Combine(process.StartInfo.WorkingDirectory, "MonoRun.exe");
                        process.StartInfo.Arguments = "\"" + Path.GetFileName(ygoMasterExePath) + "\" " + process.StartInfo.Arguments;
                    }
                    else
                    {
                        process.StartInfo.WorkingDirectory = Path.GetDirectoryName(ygoMasterExePath);
                        process.StartInfo.FileName = ygoMasterExePath;
                    }
